...
- **`sampleProducts` array** in `index.html` (~line 551) — the default model every
  ...
  - `roadmap` = year-end subscriber milestones `[{m,s}...]` at months 0/6/18/30/42
    ...
    them via `roadmapSubs()` so the 60-month curve follows the real 3-year plan (then a
    decelerating extrapolation to Dec '31 for a 5-year exit) instead
...
- **Everything is derived.** Changing a product's inputs (e.g. `subs`) auto-recomputes
  revenue, profit, ARR, margin, LTV/CAC, valuation, 60-month projection, break-even,
